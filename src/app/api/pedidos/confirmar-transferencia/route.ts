...
// Llamado desde la página /checkout/pago/transferencia cuando el usuario
// pulsa "Confirmar Pedido". Envía los emails de confirmación.
  ...
  try {
    ...
    try {
      await prisma.historialestadopedido.create({
        data: { pedidoId: id, estado: nombreEstadoTransferencia, color: colorEstadoTransferencia, fecha: new Date() },
      });
    } catch (err: any) {
    ...
    console.error("Error en confirmar-transferencia:", error);
